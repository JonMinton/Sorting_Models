...
c   
      PARAMETER (
        ...
        tract(n), # ID for areal unit
        id1(nn),id2(nn), # No longer used
        id3(n) # Indicators of
     ...
     &  pop2t(n),
        ...
        xguess(n1),
        x(n1),
     &   muguess, # Moving cost disutility parameter estimated with bisection
        mudiff,
        stayp, # stayer %
        upper, # starting values for method of bisection
        lower,
        mutol, # tolerance for method of bisection
        
     &  dist(n1,n1), # No longer used
        
        asian09(n), # vector: number of asians by tract in 2009
        black09(n),
        hisp09(n),
...
c
      # Common indicates global variable  
...
c
 ...
 555  format(i9,3f21.7)
      ...
      do i = 1,n
         ...
         pop2(i) = pop2t(i)
      enddo
      pop1(n1) = 2.d0*dabs(dpop) # population in first period (2000)
        # is set size of outside population group set to twice change of population over time
      pop2(n1) = pop1(n1)-dpop   # population in second period in rest of country in period 1 less change in population in LA
...
c
      ...
      upper = 0.1d0
      do k1 = 1,maxit
        # xguess : utility associated with each areal unit
        # fcn1: includes contraction mapping
...
c # moving cost calculation
 300  continue
     ...
     &         (k.eq.n1))) mc = 528.71d0
      ...
      # sum of area exputils for area denoms
      do j = 1,n1
        # 
         denom(j) = 0.d0
         do k = 1,n1
...
c
      temp = xavg # normalisation (needs to be done)
