...
*/
    ...
    while (repeat_loop){
        ...
        if (use_logmu) {
         ...
         Rcout << "Outer: " << repeat_nums << "\tmu: " << mu_guess;
        TimminsInnerTest(); 
        Rcout << "\tmu diff: " << mu_diff << endl;
...
*/ // I NOW THINK THE COMPARISON SHOULD BE BETWEEN STAY AND STAYP AND NOT STAYP AND PSTAY
    ...
    while(repeat_inner){
        TimminsInner();
        calls_to_inner++;
        if (change_max < tol_delta){
            Rcout << "change_max is " << change_max << " after " << calls_to_inner << " calls to inner" << endl;
            repeat_inner = false;
        }
...
//    long double sumrhs = 0.0; 
    for (int i = 0; i < n1; i++){
        ...
        for (int j = 0; j < n1; j++){
            ...
            if (isnan(rhs[i])){
                // Catch an error here
                Rcout << "error caught in rhs" << endl;
                Rcout << "###################" << endl;
                Rcout << "i:" << i << "\tj:"<< j<< endl;
                Rcout << "numerator:\t" << numer << endl;
                Rcout << "denominator:\t" << denom[j] << endl;
                Rcout << "oldcounts:\t" << oldcounts[j] << endl;
                exit(EXIT_FAILURE);
            }
        }
//        sumrhs += rhs[i];
...
/// C++ Code 
     xavg = 0;    
     long double change, rmschange = 0;
     change_max = 0;
     ...
     for (int j =0; j < n1; ++j){
         ...
         if (isnan(change)){
             // Catch an error
             Rcout << "ERROR: change value is nan\n";
             Rcout << "Status:\n";
             Rcout << "=======\n";
             Rcout << "Outer loop (mu):\t" << repeat_nums << endl;
             Rcout << "Inner loop (delta):\t" << calls_to_inner << endl;
             Rcout << "j (areal unit):\t" << j << endl;
             Rcout << "newshares:\t" << newshares[j] << "\tlog: " << log(newshares[j]) << endl;
             Rcout << "predicted:\t" << newshares_predicted[j]<< "\tlog: " << log(newshares_predicted[j]) << endl;
             Rcout << "rhs:\t" << rhs[j] << endl;
             ...
             if (dbgflag){
                 ...
                 if (abs(change) > change_max) { change_max = abs(change);}
             }
             xf[j] = x1[j] + change;
